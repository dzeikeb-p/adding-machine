// Pure-TS synchronous SHA-256. Zero runtime deps.
...
export function sha256hex(input: string): string {
  ...
  const bitLen = len * 8;
  const padLen = ((len % 64 < 56 ? 56 : 120) - (len % 64)) % 64 || 64;
  const msg = new Uint8Array(len + padLen + 8);
  msg.set(encoded);
  msg[len] = 0x80;
  // Append 64-bit big-endian bit length (only lower 32 bits needed for sane inputs)
