...
| REST API | https://adding-machine.workers.dev | Scripts, pipelines, anything that speaks HTTP |
| MCP server | https://adding-machine.workers.dev/mcp | Claude Code, Claude Desktop, Cursor, any MCP-compliant agent |
...
```bash
curl -X POST https://adding-machine.workers.dev/v1/cutup \
...
Full OpenAPI docs at https://adding-machine.workers.dev/docs.
...
```bash
npx @modelcontextprotocol/inspector https://adding-machine.workers.dev/mcp
```
